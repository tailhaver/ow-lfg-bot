...
from src.database.database import Session, Server
...
class AdminCommands(commands.Cog):
    admin_group = discord.SlashCommandGroup("admin", "administrator commands :)", default_member_permissions=discord.Permissions(administrator=True))
    ...
    @discord.option("category", type=discord.SlashCommandOptionType.channel, channel_types=[discord.ChannelType.category])
    async def admin_move_voice_category_command(
        ...
        try:
            async with Session() as db_session:
                ...
                if server_data is not None and server_data.voice_channel_category is not None:
                    previous_channel_id = server_data.voice_channel_category

                stmt = update(Server).where(Server.id == ctx.guild_id).values(voice_channel_category=category.id)
            ...
            embed = discord.Embed(
                title="❌ Unable to move voice channel category.",
                color=0xFA9C1D
            )
            await ctx.respond(embed=embed, ephemeral=True)
            return
    ...
    @discord.default_permissions(administrator=True)
    @discord.option("state", type=discord.SlashCommandOptionType.string, choices=["enable", "disable", "toggle"], default="toggle")
        ...
        try:
            async with Session() as db_session:
                ...
                if server_data is not None:
                    match state:
                        case "enable":
                            stmt = update(Server).where(Server.id == ctx.guild_id).values(logging_enabled=True)
                        case "disable":
                            stmt = update(Server).where(Server.id == ctx.guild_id).values(logging_enabled=False)
                        case _:
                            stmt = update(Server).where(Server.id == ctx.guild_id).values(logging_enabled=not server_data.logging_enabled)
                    await db_session.execute(stmt)
                    await db_session.commit()
            
            embed = discord.Embed(
                title=f"✅ Successfully {state}d logging.",
                color=0xFA9C1D
            )
            await ctx.respond(embed=embed, ephemeral=True)
        except Exception:
            embed = discord.Embed(
                title=f"❌ Unable to {state} logging.",
                color=0xFA9C1D
            )
            await ctx.respond(embed=embed, ephemeral=True)
            return
    ...
    async def admin_set_logging_channel_command(
        ...
        try:
            async with Session() as db_session:
                ...
                server_data = result.scalars().one_or_none()
                if server_data is not None:
                    stmt = update(Server).where(Server.id == ctx.guild_id).values(log_channel=int(channel.id))
                    await db_session.execute(stmt)
                    await db_session.commit()
            ...
            embed = discord.Embed(
                description="❌ Unable to change logging channel.",
                color=0xFA9C1D
            )
            await ctx.respond(embed=embed, ephemeral=True)
            return
