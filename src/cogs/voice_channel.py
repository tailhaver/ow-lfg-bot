...
from src.database.database import Session, Server, VoiceChannel
from src.bot.logging import send_log
from src.console import logger
from datetime import datetime, timezone

async def create_vc(ctx: discord.ApplicationContext, mode: str, platform: str, competitive: bool = False) -> None:
    async with Session() as db_session:
        ...
        new_vc = await ctx.guild.create_voice_channel(
            name=channel_name,
            category=discord.utils.get(ctx.guild.categories, id=server_data.voice_channel_category)
        )
...
class VCCommands(commands.Cog):
    create_voice_channel = discord.SlashCommandGroup("vc", "Create a new temporary voice channel")
    ...
    @create_voice_channel.command(
        name="delete",
        description="Delete your currently open voice channel"
    )
    ...
    async def delete_vc_command(
        ...
        async with Session() as db_session:
            if channel is not None and not ctx.author.guild_permissions.administrator:
                ...
                stmt = select(VoiceChannel).where(VoiceChannel.owner == int(ctx.author.id))
                result = await db_session.execute(stmt)
                channel_resp = result.scalars().one_or_none()
                if channel_resp is None:
                    embed = discord.Embed(
                        title="❌ You do not have any channels open!",
                        color=0xFA9C1D
                    )
                    await ctx.respond(embed=embed, ephemeral=True)
                    return
        ...
        embed = discord.Embed(
            title="✅ Successfully deleted voice channel!",
            color=0xFA9C1D
        )
        await ctx.respond(embed=embed, ephemeral=True)
        ...
        async with Session() as db_session:
            ...
            if after.channel is None: # leaving any vc
                if (channel := self.bot.get_channel(before.channel.id)) is None:
                    logger.warning(f"Bot is not able to access voice channel with ID {before.channel.id}!")
                    return
                stmt = update(VoiceChannel).where(VoiceChannel.id == channel.id).values(has_user=len(channel.members) > 0, last_leave=datetime.now(timezone.utc))
            else: # joining any vc
                if (channel := self.bot.get_channel(after.channel.id)) is None:
                    logger.warning(f"Bot is not able to access voice channel with ID {after.channel.id}!")
                    return
                stmt = update(VoiceChannel).where(VoiceChannel.id == channel.id).values(has_user=1)
    ...
    async def prune_stale_channels(self) -> None:
        async with Session() as db_session:
            ...
            for channel in channels:
                if (datetime.now(timezone.utc) - channel.last_leave.replace(tzinfo=timezone.utc)).total_seconds() < 300:
                    continue
                try:
                    voice_channel: discord.VoiceChannel = await self.bot.fetch_channel(channel.id)
        ...
        async with Session() as db_session:
            ...
            for channel in channels:
                ...
                if channel.has_user != bool(voice_members > 0):
                    stmt = update(VoiceChannel).where(VoiceChannel.id == channel.id).values(has_user=bool(voice_members > 0))
                    await db_session.execute(stmt)
            await db_session.commit()
